...
async function callOnce(prompt, options = {}) {
  ...
  if (!apiKey) throw new Error('OPENROUTER_API_KEY belum di-set di .env');
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      headers : {
        ...
        'HTTP-Referer'        : 'https://github.com/sibabu',
        'X-Title'             : 'SI BABU Agent',
      ...
      res.on('end', () => {
        try {
          ...
          if (!text) return reject(new Error('OpenRouter: response kosong'));
          resolve(text.trim());
        } catch {
    ...
    req.on('error', reject);
    req.setTimeout(60000, () => req.destroy(new Error('OpenRouter: timeout (60s)')));
    req.write(body);
    req.end();
  });
}
...
async function call(prompt, options = {}) {
  ...
  for (let i = 1; i <= MAX_RETRY; i++) {
    try {
      ...
      console.log(`[openrouter] Attempt ${i}/${MAX_RETRY} gagal: ${err.message}, retry...`);
...
async function stream(prompt, options = {}, onToken) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.setTimeout(120000, () => req.destroy(new Error('OpenRouter stream timeout')));
    req.write(body);
    req.end();
  });
}
